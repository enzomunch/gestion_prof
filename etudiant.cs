    ...
    public partial class etudiant : Form
    {
        ...
        private void etudiant_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    MySqlCommand cmd = new MySqlCommand("SELECT nomEtudiant, prenomEtudiant, mailEtudiant, telEtudiant, nomProf, nomClasse from etudiants INNER JOIN professeurs ON professeurs.numProf = etudiants.numProf INNER JOIN classes ON classes.numClasse = etudiants.numClasse", connexion_etudiants);
                    // Execution de la requete 
                    MySqlDataReader grille_etudiants = cmd.ExecuteReader();
                    //
        ...
        {
            MySqlConnection connexion_recherche = new MySqlConnection("database=gestion_prof; server=localhost; user id =root; mdp=");
            connexion_recherche.Open();

            if(Globals.keyProf == 13)
            {
                MySqlCommand cmd = new MySqlCommand("SELECT nomEtudiant, prenomEtudiant, mailEtudiant, telEtudiant, nomProf, nomClasse from etudiants INNER JOIN professeurs ON professeurs.numProf = etudiants.numProf INNER JOIN classes ON classes.numClasse = etudiants.numClasse WHERE nomEtudiant LIKE '%" + textNom.Text + "%'", connexion_recherche);
                ...
                recherche.Close();
                connexion_recherche.Close();
            }
            else
            {
                MySqlCommand cmd = new MySqlCommand("SELECT nomEtudiant, prenomEtudiant, mailEtudiant, telEtudiant, nomProf, nomClasse from etudiants INNER JOIN professeurs ON professeurs.numProf = etudiants.numProf INNER JOIN classes ON classes.numClasse = etudiants.numClasse WHERE nomEtudiant LIKE '%" + textNom.Text + "%' AND etudiants.numProf = " + Globals.keyProf, connexion_recherche);
        ...
        private void btnReinitialiser_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                // Requête pour récupérer la liste des étudiants
                connexion.Open();
                MySqlCommand cmd = new MySqlCommand("SELECT nomEtudiant, prenomEtudiant, mailEtudiant, telEtudiant, nomProf, nomClasse from etudiants INNER JOIN professeurs ON professeurs.numProf = etudiants.numProf INNER JOIN classes ON classes.numClasse = etudiants.numClasse", connexion);
                // Execution de la requete 
                MySqlDataReader grille_etudiants = cmd.ExecuteReader();
                //
                dataGridViewEtudiant.Rows.Clear();
